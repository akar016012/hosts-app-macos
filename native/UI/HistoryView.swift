...
struct HistorySheet: View {
    ...
    private func confirmClear() {
        let alert = NSAlert()
        alert.messageText = "Clear change history?"
        alert.informativeText = "This permanently removes all saved versions. The current file is kept as a new baseline. This can't be undone."
        alert.addButton(withTitle: "Clear history")
        alert.addButton(withTitle: "Cancel")
        alert.alertStyle = .warning
        if alert.runModal() == .alertFirstButtonReturn {
            store.clearHistory()
            selectedID = store.history.first?.id
        }
    }
}
